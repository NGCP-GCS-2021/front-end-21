<template>
  <div>
    <v-container>
      <v-row>
        <v-col>
          <h2 class="font-weight-light" pa-0 ma-0>Keep Out Area: {{ vehicle }}</h2>
        </v-col>
      </v-row>
      ...
      <v-card>
        <v-card-title class="headline">
          ...
          <h3 class="font-weight-light text-center red--text" justify="center">
            Clear Keep Out: {{ vehicle }}?
          </h3>
        </v-card-title>
        <v-card-text justify="center">
          Warning: Current {{ vehicle }} Geofence (Keep Out) inputs
          will be cleared.
        </v-card-text>
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$emit("addToKeepOut", this.Coordinates);
      this.clear();
    },
    add() {
