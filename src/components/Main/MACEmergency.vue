  ...
  methods: {
        ...
        .then((res) => {
          ...
          for (let i = 0; i < this.mac_data.length; i++) {
            if (this.mac_data[i].title == "Latitude") {
              this.e_stop.E_stop_lat = this.mac_data[i].value;
            } else if (this.mac_data[i].title == "Longitude") {
              this.e_stop.E_stop_lng = this.mac_data[i].value;
            } else if (this.mac_data[i].title == "Altitude") {
              this.e_stop.E_stop_alt = this.mac_data[i].value;
            }
          }
    ...
    postEmergencyStop() {
      ...
      axios
        ...
        .then(() => {
          console.log(
            "Posted Emergency Stop Command and Coordinates to MAC_INPUT"
          );
          this.activated = true;
